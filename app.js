...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateThemeColor(e) {
        if(themeColorMeta) themeColorMeta.setAttribute('content', e.matches ? '#000000' : '#f2f2f7');
    }
    if(themeColorMeta) {
    ...
    window.openGacha = function() {
        ...
        const timer = setInterval(() => {
            ...
            if(count > 12) {
                ...
                res.style.transition = 'transform 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275)';
    ...
    window.openLanguageModal = function() {
        const phrases = [
            ...
            {ja: 'これをお願いします', tw: '請幫我點這個 (指菜單)', ro: 'Kore o onegaishimasu'},
            {ja: 'お会計をお願いします', tw: '麻煩結帳', ro: 'Okaikei o onegaishimasu'},
            {ja: 'トイレはどこですか？', tw: '請問廁所在哪裡？', ro: 'Toire wa doko desu ka?'}
        ];
    ...
    async function fetchLiveInfo() {
        ...
        try {
            ...
            const weatherRes = await fetch('https://api.open-meteo.com/v1/forecast?latitude=34.6937&longitude=135.5022&current=temperature_2m,weathercode&timezone=Asia%2FTokyo', { signal: AbortSignal.timeout(5000) });
            const weatherData = await weatherRes.json();
            const temp = Math.round(weatherData.current.temperature_2m);
            
            // Free API: Exchange Rate Converter (TWD base to get JPY value)
            const rateRes = await fetch('https://open.er-api.com/v6/latest/TWD', { signal: AbortSignal.timeout(5000) });
            ...
            localStorage.setItem('osaka-live-time', new Date().toLocaleTimeString('zh-TW', {hour: '2-digit', minute:'2-digit'}));
            
        } catch (e) {
            const cached = localStorage.getItem('osaka-live-cache');
            const cacheTime = localStorage.getItem('osaka-live-time');
            if (cached) {
                badge.innerHTML = cached.replace('</span>', ` <span style="font-size:10px; opacity:0.7;">(上次更新 ${cacheTime})</span></span>`);
            } else {
    ...
    if (trafficBanner) {
        ...
        setTimeout(() => {
            ...
            if (isDelayed) {
                ...
                trafficBanner.innerHTML = '<i class="fa-solid fa-triangle-exclamation"></i> <span>【行車情報】JR 大阪環狀線部分列車延誤，請預留充裕時間。</span>';
            } else {
                trafficBanner.style.background = '#34c759'; // Green
                trafficBanner.innerHTML = '<i class="fa-solid fa-circle-check"></i> <span>【即時交通】目前關西所有地鐵、JR線運行正常。</span>';
    ...
    const datePattern = new RegExp(`(${todayMonth}/${todayDate}|0?${todayMonth}/0?${todayDate})`);
    ...
    function renderItinerary() {
        ...
        if (activeTab) {
            const centerPos = activeTab.offsetLeft - (dayTabsContainer.offsetWidth / 2) + (activeTab.offsetWidth / 2);
    ...
    function renderChecklist() {
        ...
        const savedState = JSON.parse(localStorage.getItem('osaka-checklist') || '{}');
        ...
        for (const [category, items] of Object.entries(window.checklistData)) {
            ...
            items.forEach(item => {
                ...
                label.innerHTML = item.name + (item.required ? ' <span class="req-badge">必備</span>' : '');
    ...
    function initMap() {
        ...
        if (!navigator.onLine) {
            setTimeout(() => {
                showModal("離線狀態提示", "目前沒有網路連線，地圖圖層可能無法正常顯示。部分地點導航資訊會失效。");
            }, 1000);
        }
